...
char JsonReader::currChar() const { return src_str[curr_pos]; }

char JsonReader::getChar() {
  if (eof_flag) {
    throw JsonError("Json file ends unexpectedly!", getCurrLine(),
                    getCurrCol());
  }
...
std::string JsonReader::strAhead(int n) const {
  return src_str.substr(curr_pos, std::min<int>(n, src_str_len - curr_pos));
}
...
int JsonReader::getCurrLine() const { return curr_line; }

int JsonReader::getCurrCol() const { return curr_col; }
...
bool JsonReader::isEOF() { return eof_flag; }
...
JsonNode::JsonNode(JsonValueType _value_type, const std::string &_key) {
...
int JsonNode::get_value_type() const { return value_type; }
...
void JsonNode::set_key(const std::string &_key) { key = _key; }

std::string JsonNode::get_key() const { return key; }
...
JsonNode &JsonNode::obj_get_item(const std::string &_key) {
  if (value_type != JSON_VALUE_TYPE_OBJECT) {
    ...
    throw JsonError("No item named " + _key, 0, 0, false);
  }
}
...
void JsonNode::toJsonStr(std::string &_dst_str, bool _print_key, int _tab_num,
  ...
  case JSON_VALUE_TYPE_ARRAY: {
    ...
    while (curr) {
      if (_one_line_per_element) {
        ...
        curr->toJsonStr(element_str, false, _tab_num + 1, true);
        _dst_str += element_str;
      } else {
        std::string element_str;
        curr->toJsonStr(element_str, false, 0, false);
        _dst_str += element_str;
      }
  ...
  case JSON_VALUE_TYPE_OBJECT: {
    ...
    while (curr) {
      if (_one_line_per_element) {
        _dst_str += '\n';
        std::string element_str;
        curr->toJsonStr(element_str, true, _tab_num + 1, true);
        _dst_str += element_str;
      } else {
        std::string element_str;
        curr->toJsonStr(element_str, true, 0, false);
        _dst_str += element_str;
      }
...
JsonNode &JsonNode::operator[](const std::string &_key) {
  ...
  if (value_type != JSON_VALUE_TYPE_OBJECT)
    throw JsonError("Value type mismatched!", 0, 0, false);

  if (obj_has_item(_key))
    return obj_get_item(_key);
  else {
    JsonNode_P _new_item(new JsonNode(JSON_VALUE_TYPE_NULL, _key));
...
JsonNode &JsonNode::operator[](int _array_ind) {
  ...
  if (_array_ind < 0 || _array_ind >= children_size)
    throw JsonError("Array index out of bound!", 0, 0, false);
  else
...
double JsonNode::operator=(double _num_val) {
  ...
  if (value_type == JSON_VALUE_TYPE_DOUBLE) {
    ...
    throw JsonError("Cannot pass double value to this node!", 0, 0, false);
  }
}
...
std::string JsonNode::operator=(const std::string &_str_val) {
  ...
  if (value_type == JSON_VALUE_TYPE_STRING) {
    ...
    throw JsonError("Cannot pass string value to this node!", 0, 0, false);
  }
}
...
int JsonNode::as_int() const {
  ...
  default:
    throw JsonError("Value is not convertible to Int!", 0, 0, false);
    break;
  }
}
...
double JsonNode::as_double() const {
  ...
  default:
    throw JsonError("Value is not convertible to Double!", 0, 0, false);
    break;
  }
}
...
bool JsonNode::as_bool() const {
  ...
  default:
    throw JsonError("Value is not convertible to Bool!", 0, 0, false);
    break;
  }
}
...
std::string JsonNode::as_string() const {
  ...
  default:
    throw JsonError("Value is not convertible to String!", 0, 0, false);
    break;
  }
}
...
void JsonNode::setValueType(int _value_type) { value_type = _value_type; }

/******************************class Json**************************/

Json::Json() { root = std::make_shared<JsonNode>(JSON_VALUE_TYPE_OBJECT); }
...
JsonNode &Json::getRoot() { return *root; }

bool Json::succeed() const { return success; }

bool Json::fail() const { return !success; }
...
void Json::parseValue(JsonNode_P _node, JsonReader &_reader) {
  ...
  if (ch == '{')
    ...
    if (_reader.strAhead(5) == "false") {
      ...
      throw JsonError("Invalid Symbol:" + ch, _reader.getCurrLine(),
                      _reader.getCurrCol());
    }
  }
}
...
void Json::parseNumber(JsonNode_P _node, JsonReader &_reader) {
  ...
  if (curr_char != '-' && !isdigit(curr_char)) {
    throw JsonError("Invalid Symbol:" + curr_char, _reader.getCurrLine(),
                    _reader.getCurrCol());
  }
  if (curr_char == '-') {
    sign = -1;
  ...
  if (curr_char == '.') {
    ...
    if (!isdigit(curr_char)) {
      ...
      while (isdigit(curr_char)) {
        sub_scale += 1;
        value =
            value + double(curr_char - '0') * std::pow(0.1, double(sub_scale));
        _reader.getChar();
        curr_char = _reader.currChar();
      }
    }
...
void Json::parseString(JsonNode_P _node, JsonReader &_reader) {
  ...
  if (curr_char != '\"') {
    throw JsonError("Invalid string start symbol:" + curr_char,
                    _reader.getCurrLine(), _reader.getCurrCol());
  } else {
    _reader.getChar();
    curr_char = _reader.currChar();
  }
  ...
  while (curr_char != '\"') {
    if (curr_char == '\\') // escaped character
    {
      ...
      if (curr_char == '\"' || curr_char == '\\' || curr_char == '/' ||
          curr_char == '\b' || curr_char == '\f' || curr_char == '\n' ||
      ...
      else if (curr_char == 'u') // unicode character \uxxxx
      {
        ...
        for (int i = 0; i < 4; i++) {
          ...
          if ((curr_char >= '0' && curr_char <= '9') ||
            ...
            throw JsonError("Invalid unicode string symbol: " + curr_char,
                            _reader.getCurrLine(), _reader.getCurrCol());
          }
        }
...
void Json::toString(std::string &_output_str, bool _one_line_per_item) const {
  _output_str.clear();
